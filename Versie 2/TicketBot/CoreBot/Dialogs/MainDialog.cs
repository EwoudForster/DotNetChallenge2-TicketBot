...
namespace CoreBot.DialogDetails
{
	public class MainDialog : ComponentDialog
	{
		...
		public MainDialog(CheckScheduleDialog checkScheduleDialog, BookTicketDialog bookTicketDialog, RatingDialog ratingDialog, TicketBotCLURecognizer cluRecognizer, ILogger<MainDialog> logger)
		...
		private async Task<DialogTurnResult> HandleOptionStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
		{
			...
			if (_cluRecognizer.IsConfigured)
			{
				...
				{
					// Handle case where recognition result is null
					await stepContext.Context.SendActivityAsync(MessageFactory.Text("Sorry, I couldn't recognize your request."), cancellationToken);
